...
def grade_generation_grounded_in_documents_and_question(state: GraphState) -> str:
    ...
    if generated_answer is not None:
        score = hallucination_grader.invoke(
            {"documents": documents, "generation": generated_answer}
        )
        if hallucination_grade := score.binary_score:
            print("==== DECISION: GENERATION IS GROUNDED IN DOCUMENTS ====")
            print("==== GRADE GENERATION vs QUESTION ====")
            score = answer_grader.invoke(
                {"question": question, "generation": generated_answer}
            )
...
flow.add_conditional_edges(
    GRADE_DOCS, decide_to_generate, {WEB_SEARCH: WEB_SEARCH, GENERATE: GENERATE}
)
...
flow.add_conditional_edges(
    ...
    {"not_supported": GENERATE, "useful": END, "not_useful": WEB_SEARCH},
)
