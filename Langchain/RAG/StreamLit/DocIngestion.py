...
async def index_document_async(documents: List[Document], batch_size: int = 50):
    ...
    async def add_batch(batches: List[Document], batch_num: int):
        try:
            ...
            print(
                f">>> Vector store index -> Successfully added batch {batch_num}/{len(batches)} ({len(batches)} documents)"
            )
    ...
    if successful == len(batches):
        print(
            f"All batches processed successfully in total : {successful}/{len(batches)}"
        )
    else:
